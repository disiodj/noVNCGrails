    ...
    * ------v------*/
    ...
    updateVisualState(state) {
        ...
        switch (state) {
            ...
            case 'connecting':
                transitionElem.textContent = (0, _localization.default)("Connecting...");
            ...
            case 'disconnecting':
                transitionElem.textContent = (0, _localization.default)("Disconnecting...");
            ...
            case 'reconnecting':
                transitionElem.textContent = (0, _localization.default)("Reconnecting...");
    ...
    dragControlbarHandle(e) {
        ...
        const ptr = (0, _events.getPointerEvent)(e);
        const anchor = document.getElementById('noVNC_control_bar_anchor');
    ...
    moveControlbarHandle(viewportRelativeY) {
        ...
        const margin = 10; // These heights need to be non-zero for the below logic to work

        if (handleHeight === 0 || controlbarBounds.height === 0) {
            return;
        }

        let newY = viewportRelativeY; // Check if the coordinates are outside the control bar
    ...
    controlbarHandleMouseDown(e) {
        ...
        const ptr = (0, _events.getPointerEvent)(e);
        const handle = document.getElementById("noVNC_control_bar_handle");
        const bounds = handle.getBoundingClientRect(); // Touch events have implicit capture
    ...
    disconnectFinished(e) {
        const wasConnected = UI.connected; // This variable is ideally set when disconnection starts, but
    ...
    securityFailed(e) {
        let msg = ""; // On security failures we might get a string with a reason
    ...
    setCredentials(e) {
        ...
        const password = inputElemPassword.value; // Clear the input after reading the password
    ...
    showVirtualKeyboard() {
        ...
        try {
            const l = input.value.length; // Move the caret to the end
    ...
    keepVirtualKeyboard(event) {
        const input = document.getElementById('noVNC_keyboardinput'); // Only prevent focus change if the virtual keyboard is active
    ...
    keyInput(event) {
        ...
        let backspaces = inputs < 0 ? -inputs : 0; // Compare the old string with the new to account for
        ...
        for (let i = newLen - inputs; i < newLen; i++) {
            UI.rfb.sendKey(_keysymdef.default.lookup(newValue.charCodeAt(i)));
    ...
    bell(e) {
        if (WebUtil.getConfigVar('bell', 'on') === 'on') {
            const promise = document.getElementById('noVNC_bell').play(); // The standards disagree on the return value here
...
if (_localization.l10n.language === "en" || _localization.l10n.dictionary !== undefined) {
    ...
    fetch('app/locale/' + _localization.l10n.language + '.json').then(response => {
        if (!response.ok) {
            throw Error("" + response.status + " " + response.statusText);
        }
    ...
    }).catch(err => Log.Error("Failed to load translations: " + err)).then(UI.prime);
}
