...
function constructor() {
    ...
    ws.on('close', function(e) {
        ...
        if (rfb_state === 'disconnect') {
            ...
            $("#noVNC_status").text("Server disconnected");
...
updateState = function(state, statusMsg) {
    var message = '', func, cmsg, oldstate = rfb_state;
    
    var messageMap =
    {
        "noVNC ready: native WebSockets": "Seting up connection",
        "Starting VNC handshake": "Establishing connection",
        "Sent ProtocolVersion": "Establishing connection",
        "Authenticating using scheme": "Establishing connection",
        "WebSock error": "Connection failed",
        "Connect timeout": "Connection timeout",
        "Authentication OK": "Authenticated",
        "Connected (unencrypted)": "Connected",
        "Disconnecting": "Disconnecting",
        "VNC disconnected": "Disconnected"
    };
    
    $.each(messageMap, function(key, val) {
    	if (statusMsg != null && statusMsg.slice(0, key.length) == key) {
    		if(val == 'Connected') {
    			$("#noVNC_status").hide();
    		} else {
    			message = val;
    ...
    if ((oldstate === 'failed') && (state === 'disconnected')) {
        ...
        conf.updateState(that, state, oldstate, message); // Obsolete
        conf.onUpdateState(that, state, oldstate, message);
    }
};
